<script lang="ts">
	import { Image, MousePointerClick, Type } from '@lucide/svelte';
...
<div class="rs">
	...
	<div class="rs__letter">
		<div class="rs__zone" data-zone="header" class:is-over={over === 'header'}>
			{placed.header ? blocks.find((b) => b.id === placed.header)?.label : zones[0].label}
		</div>
		<p class="rs__text">
			Привет! Спасибо, что подписались на новости кофейни «Зерно». Раз в неделю мы будем присылать
			новинки меню и скидки для своих.
		</p>
		<div class="rs__zone" data-zone="afterText" class:is-over={over === 'afterText'}>
			{#if placed.afterText === 'button'}<span class="rs__cta">Забрать скидку</span
				>{:else}{placed.afterText
					? blocks.find((b) => b.id === placed.afterText)?.label
					: zones[1].label}{/if}
		</div>
		<div class="rs__zone rs__zone--small" data-zone="footer" class:is-over={over === 'footer'}>
			{placed.footer ? blocks.find((b) => b.id === placed.footer)?.label : zones[2].label}
		</div>
	</div>
</div>

{#if dragging}
	<div class="rs__ghost" style="left: {pos.x}px; top: {pos.y}px">
		{blocks.find((b) => b.id === dragging)?.label}
	</div>
{/if}
